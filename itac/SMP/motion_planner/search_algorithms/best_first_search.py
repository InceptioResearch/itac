...
class BestFirstSearch(SearchBaseClass, ABC):
    ...
    def execute_search(self, timeout_sec: int = 30, vehicle_type: str = None) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        """
        ...
        while not self.frontier.empty():
            ...
            if (current_time - start_time).seconds > timeout_sec:
                print('search time too long time out, change search time limit, break!!')
            ...
            for primitive_successor in node_current.get_successors():
                ...
                if not self.is_collision_free(path_translated, vehicle_type):
