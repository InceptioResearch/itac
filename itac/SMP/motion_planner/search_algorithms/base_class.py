...
from SMP.maneuver_automaton.motion_primitive_semi_trailer import TruckMotionPrimitive
...
class SearchBaseClass(ABC):
    ...
    def __init__(self, scenario: Scenario, planningProblem: PlanningProblem, automaton: 'ManeuverAutomaton|TruckManeuverAutomaton',
        ...
        self.shape_ego_trailer: Rectangle = None
        if self.automaton.type_vehicle == VehicleType.SEMI_TRAILER:
            self.shape_ego_trailer = automaton.shape_ego_trailer
        ...
        self.motion_primitive_initial = automaton.create_initial_motion_primitive(
            planningProblem)
        ...
        self.collision_checker.add_collision_object(
            shapegroup_triangles_boundary)
    ...
    def parse_planning_problem(self) -> None:
        ...
        """
        assert isinstance(
            self.planningProblem, PlanningProblem), "Given planning problem is not valid!"
        ...
        if hasattr(self.planningProblem.goal.state_list[0], 'position'):
            if hasattr(self.planningProblem.goal.state_list[0].position, 'vertices'):
                ...
                self.position_desired = [
                    Interval(start=x, end=x), Interval(start=y, end=y)]
    ...
    def initialize_lanelets_costs(self) -> None:
        ...
        if self.list_ids_lanelets_goal is not None:
            ...
            for ids_lanelet_goal in self.list_ids_lanelets_goal:
                ...
                lanelet_goal = self.scenario.lanelet_network.find_lanelet_by_id(
                    ids_lanelet_goal)
    ...
    def calc_angle_of_position(centerVertices: np.ndarray, pos: np.ndarray) -> float:
        ...
        """
        index_closestVert = SearchBaseClass.find_closest_vertex(
            centerVertices, pos)
    ...
    def calc_dist_to_closets_point_on_line(vertexA: np.ndarray, vertexB: np.ndarray, pos: np.ndarray) -> float:
        ...
        magAB2 = (vertexB[0] - vertexA[0]) ** 2 + \
            (vertexB[1] - vertexA[1]) ** 2
        ABdotAP = (pos[0] - vertexA[0]) * (vertexB[0] - vertexA[0]) + \
            (pos[1] - vertexA[1]) * (vertexB[1] - vertexA[1])
        s = ABdotAP / magAB2
        if s < 0:
    ...
    def calc_distance_to_nearest_point(centerVertices: np.ndarray, pos: np.ndarray) -> float:
        ...
        if (index_closestVert + 1) < len(centerVertices):
            ...
            dist1 = SearchBaseClass.distance(
                centerVertices[index_closestVert], pos, 0)
        ...
        if index_closestVert > 0:
            ...
            dist2 = SearchBaseClass.distance(
                centerVertices[index_closestVert], pos, 0)
    ...
    def calc_travelled_distance(path: List[State]) -> float:
        ...
        for i in range(len(path) - 1):
            dist += SearchBaseClass.distance(path[i].position,
                                             path[i + 1].position)
        return dist
    ...
    def calc_curvature_of_polyline(polyline: np.ndarray) -> float:
        ...
        curvatureArray = np.abs(
            d2x_dt2 * dy_dt - dx_dt * d2y_dt2) / (dx_dt * dx_dt + dy_dt * dy_dt) ** 1.5
        curvature = 0
    ...
    def calc_lanelet_cost(self, lanelet_current: Lanelet, dist: int, list_lanelets_visited: List[int]) -> None:
        ...
        if lanelet_current.predecessor is not None:
            ...
            for pred in lanelet_current.predecessor:
                self.calc_lanelet_cost(self.lanelet_network.find_lanelet_by_id(
                    pred), dist + 1, list_lanelets_visited)
        ...
        for obstacleObj in obstacles_in_lanelet:
            ...
            if isinstance(shape_obs, Circle):
                if SearchBaseClass.distance(pos, shape_obs.center) < shortestDist:
                    shortestDist = SearchBaseClass.distance(
                        pos, shape_obs.center)
        return shortestDist
    ...
    def calc_heuristic_lanelet(self, path: List[State]) -> Union[Tuple[None, None, None], Tuple[float, list, list]]:
        ...
        for i in range(len(path)):
            ...
            if not lanelets_of_pathSegment:
                # return none if path element is not in a lanelet with correct orientation
                return None, None, None
            laneletObj = self.scenario.lanelet_network.find_lanelet_by_id(
                lanelets_of_pathSegment[0])
    ...
    def is_collision_free(self, path: List[State], vehicle_type: str = None) -> bool:
        """
        ...
        traj_pred = TrajectoryPrediction(
            trajectory=trajectory, shape=self.shape_ego, vehicle_type=vehicle_type)
