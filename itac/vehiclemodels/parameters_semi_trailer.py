...
def parameters_semi_trailer():
    """
    parameters_semi_trailer - parameter set of the kinematic single track model with on-axle trailer (truck model)
    ...
    p.steering.min = -0.9  # minimum steering angle [rad]
    p.steering.max = 0.9  # maximum steering angle [rad]
    ...
    p.longitudinal.v_max = 22.22  # maximum velocity [m/s]   # approximate value: 80 km/h
